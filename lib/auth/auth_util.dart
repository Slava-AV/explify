...
// int get currentUserPageLimit => currentUser?.user?.pageLimit ?? 0;
...
Future get pageLimit async {
  CollectionReference _collectionRef =
      FirebaseFirestore.instance.collection('users');
  QuerySnapshot querySnapshot =
      await _collectionRef.where('uid', isEqualTo: currentUserUid).get();
  final userList = querySnapshot.docs
      .map((doc) => doc.data() as Map<String, dynamic>)
      .toList();
  final user = userList.first;
  print("User: $user");
  return user["pageLimit"];
}

Future get userVoicesList async {
  CollectionReference _collectionRef =
      FirebaseFirestore.instance.collection('users');
  QuerySnapshot querySnapshot =
      await _collectionRef.where('uid', isEqualTo: currentUserUid).get();
